...
/**
 * Murf AI provider — enterprise-grade text-to-speech synthesis.
 *
 * <p>API: {@code https://api.murf.ai/v1}
 * <p>Supports: TEXT_TO_SPEECH
 * <p>Auth: Bearer token
 *
 * <p>TTS: POST /speech/generate
 * <p>Request: JSON {@code {"text": "...", "voiceId": "...", "model": "..."}}
 * <p>Response: binary audio bytes
 *
 * <p>Big O: O(1) per request — synchronous, no polling.
 */
@Component
@ConditionalOnProperty(name = "app.ai.providers.murf.api-key")
public class MurfApiProvider extends AbstractAudioProvider
        implements TextToSpeechCapable {

    private static final String PROVIDER_NAME = "murf";
    private static final Set<Capability> SUPPORTED_CAPABILITIES =
            EnumSet.of(Capability.TEXT_TO_SPEECH);

    public MurfApiProvider(
            ObjectMapper objectMapper,
            CircuitBreakerRegistry circuitBreakerRegistry,
            @Value("${app.ai.providers.murf.api-key:}") String apiKey,
            @Value("${app.ai.providers.murf.base-url:https://api.murf.ai/v1}") String baseUrl,
            @Value("${app.ai.providers.murf.timeout-ms:60000}") long timeoutMs,
            @Value("${app.ai.providers.murf.retry-attempts:2}") int retryAttempts,
            @Value("${app.ai.providers.murf.retry-backoff-ms:1000}") long retryBackoffMs,
            @Value("${app.ai.providers.murf.supported-models:murf-pro,murf-enterprise}") List<String> supportedModels
    ) {
        super(objectMapper,
                circuitBreakerRegistry.circuitBreaker("aiProviderMurf"),
    ...
    public SynthesisResult synthesize(TextToSpeechRequest request) {
        ...
        String endpoint = baseUrl + "/speech/generate";

        Map<String, Object> body = new LinkedHashMap<>();
        body.put("text", request.text());
        body.put("model", model);
        if (request.voice() != null) {
            body.put("voiceId", request.voice());
        }
