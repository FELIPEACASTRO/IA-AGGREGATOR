...
public class GroqModelProvider implements AiModelProvider {
    ...
    public String generate(String prompt, String model) {
        if (!supports(model)) {
            throw new TechnicalException(ErrorCode.AI_007, "Groq provider is not configured for model: " + model);
        }

        Supplier<String> guardedCall = CircuitBreaker.decorateSupplier(circuitBreaker, () -> callGroq(prompt, model));
    ...
    private void sleepBackoff() {
        ...
        try {
            ...
            throw new TechnicalException(ErrorCode.AI_002, "Groq retry interrupted", interruptedException);
        }
    }

    private String callGroq(String prompt, String model) {
        try {
            String payload = objectMapper.writeValueAsString(new GroqRequest(model, prompt));
            ...
            if (response.statusCode() >= 400) {
                throw new TechnicalException(ErrorCode.AI_007, "Groq rejected request with status " + response.statusCode());
            }
    ...
    private record GroqRequest(String model, List<GroqMessage> messages) {
        private GroqRequest(String model, String prompt) {
            this(model, List.of(new GroqMessage("user", prompt)));
        }
    }
