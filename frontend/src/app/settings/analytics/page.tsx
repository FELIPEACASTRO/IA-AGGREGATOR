...
export default function AnalyticsDiagnosticsPage() {
  ...
  const loadPersistedReports = async (options?: { append?: boolean; page?: number }) => {
    ...
    try {
      ...
      const response = await api.get(endpoint.replace(`${process.env.NEXT_PUBLIC_API_URL || ''}/api/v1`, ''));
      const payload = response.data as { data?: PersistedReport[] };
  ...
  const loadReportEvents = async (
    ...
    try {
      ...
      const response = await api.get(endpoint.replace(`${process.env.NEXT_PUBLIC_API_URL || ''}/api/v1`, ''));
      const payload = response.data as { data?: PersistedReportEvent[] };
