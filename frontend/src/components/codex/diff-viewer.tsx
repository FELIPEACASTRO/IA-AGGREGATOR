...
export function DiffViewer({ snapshot }: { snapshot: DiffSnapshot | null }) {
  ...
  return (
    <section className="grid gap-3 lg:grid-cols-[280px_1fr]">
      <aside className="rounded-[var(--radius-2xl)] border border-[var(--border)] bg-[var(--surface)] p-3">
        ...
        <div className="mt-3 space-y-2">
          ...
          {files.map((file) => (
            <button
              ...
              className={`w-full rounded-[var(--radius-lg)] border px-3 py-2 text-left text-xs ${selected?.id === file.id ? 'border-[var(--border-strong)] bg-[var(--surface-hover)] text-[var(--foreground)]' : 'border-[var(--border)]'}`}
            >
      ...
      <div className="rounded-[var(--radius-2xl)] border border-[var(--border)] bg-[var(--surface-active)] p-3">
        ...
        {selected && (
          <div className="max-h-[60vh] overflow-auto space-y-3">
            {selected.hunks.length === 0 && (
              <pre className="rounded-[var(--radius-lg)] border border-[var(--border)] bg-[var(--surface-active)] p-3 text-xs text-[var(--muted-foreground)]">
                {snapshot?.patch || 'Sem patch disponivel'}
              </pre>
            )}
            {selected.hunks.map((hunk) => (
              <pre key={hunk.id} className="rounded-[var(--radius-lg)] border border-[var(--border)] bg-[var(--surface-active)] p-3 text-xs leading-6 text-[var(--muted-foreground)] whitespace-pre-wrap">
                {hunk.content}
              </pre>
            ))}
          </div>
